    ...
    <nav class="sticky top-0 z-30 border-b border-slate-200/70 bg-white/80 backdrop-blur-md">
        <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
            ...
            <div class="flex items-center gap-3">
                ...
                @auth
                    @php
                        $helpRoute = $user?->role === \App\Models\User::ROLE_PARTICIPANT ? 'participant.help'
                            : ($user?->role === \App\Models\User::ROLE_FACILITATOR ? 'mentor.help' : null);
                    @endphp
                    @if($helpRoute)
                        <a href="{{ route($helpRoute) }}" title="{{ __('Ayuda') }}"
                           class="grid h-9 w-9 place-items-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-brand-600 {{ request()->routeIs($helpRoute) ? 'bg-slate-100 text-brand-600' : '' }}">
