@php
    ...
    $mode = $mode ?? 'mentee';
@endphp
{{-- Per-session join link + materials + survey.
     Expects $meetingUrl, $sessTools, $surveyUrl, $mode ('mentor'|'mentee') --}}
@if($meetingUrl || $sessTools->isNotEmpty() || $surveyUrl)
    <div class="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
        @if($meetingUrl)
            <a href="{{ $meetingUrl }}" target="_blank" rel="noopener"
               class="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-brand-700">
                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25h-9A2.25 2.25 0 002.25 7.5v9a2.25 2.25 0 002.25 2.25z"/></svg>
                {{ __('Ingresar a la sesión') }}
            </a>
        @endif

        @foreach ($sessTools as $tool)
        ...
        @if($surveyUrl)
            ...
            @if($mode === 'mentor')
                <button type="button" data-copied="{{ __('¡Copiado!') }}"
                        onclick="pmCopy(@js($surveyUrl), this)"
                        class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-brand-200 hover:text-brand-600">
